...
import { updateTasks } from './task-store.mjs';
import { updateMeetings, MEETINGS_VERSION_KEY, MeetingStoreError } from './capture-idempotency.mjs';

// Compare-and-set writes for `meetings` (defined next to the capture commit script that bumps the
// same version counter). Every meeting mutation goes through updateMeetings; saveMeetings is the raw
// write kept only for one-off migration scripts.
export { updateMeetings, MEETINGS_VERSION_KEY, MeetingStoreError };
 ...
 * independent key), then the metadata arrays are prepended under compare-and-set
 * so a concurrent capture or website edit cannot drop them.
...
export async function addMeetingWithTasks(meeting, newTasks = []) {
  ...
  await updateMeetings(kv, meetings => ({ meetings: [meta, ...meetings] }));
  await updateTasks(kv, tasks => ({ tasks: [...newTasks, ...tasks] }));
  return meta;
}

/** Remove a meeting record and its transcript key. Caller handles tasks.
 *  The array argument is ignored in favour of a compare-and-set read (kept for callers' signatures). */
export async function removeMeeting(_meetings, meetingId) {
  const outcome = await updateMeetings(kv, meetings => ({ meetings: meetings.filter(m => m.id !== meetingId) }));
  await deleteTranscript(meetingId);
  return outcome.meetings;
}
