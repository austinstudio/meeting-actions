...
/** Trashing a task that was still in the inbox also clears it ("Not mine" in the app, delete on the web). */
export function isInboxDismissal(entry, task) {
  return Boolean(entry) && entry.type === 'delete' && Boolean(task) && task.status === 'uncategorized';
}
...
export function computeTriageStats(tasks, userId, { now = new Date(), tz = 'UTC', days = 30 } = {}) {
  ...
  for (const task of Array.isArray(tasks) ? tasks : []) {
    ...
    for (const entry of task.activity) {
      if (!isInboxClear(entry) && !isInboxDismissal(entry, task)) continue;
