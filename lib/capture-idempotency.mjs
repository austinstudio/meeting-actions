...
import { TASKS_CAS_SCRIPT, TASKS_VERSION_KEY } from './task-store.mjs';
import { captureSourceID, captureSourceMeta, dailyMeetingSuffix } from './capture-sources.mjs';
...
// ---------------------------------------------------------------------------
// Compare-and-set writes for the shared `meetings` array (mirror of lib/task-store.mjs).
//
// Meeting writers used to read the whole array, edit it and write it back. A capture committed
// between that read and write (captures append atomically in Lua) was silently erased, and the
// next capture then treated the day's meeting as new. Writers now read `meetings:version`, read
// the array, compute the new array, and commit with the same Lua compare-and-set as tasks; the
// capture commit script increments this counter too. Lives in this .mjs so the node test suite
// can import it; lib/meeting-store.js re-exports it for the routes.
export const MEETINGS_KEY = 'meetings';
export const MEETINGS_VERSION_KEY = 'meetings:version';
export const MEETINGS_CAS_SCRIPT = TASKS_CAS_SCRIPT;   // identical script, different keys
...
/**
 * Run `mutate(meetings)` against the current array and commit atomically. `mutate` may be async
 * and returns `{ meetings, ...anything }` to write, or `null` to write nothing. Retries on
 * conflict; throws MeetingStoreError ('meetings_write_conflict' | 'meetings_too_large' | 'meetings_corrupt').
 */
export async function updateMeetings(kv, mutate, { attempts = 6 } = {}) {
  for (let attempt = 0; attempt < attempts; attempt++) {
    // Version first, then the array: a write landing between the two reads fails the CAS (stale version).
    const version = (await kv.get(MEETINGS_VERSION_KEY)) ?? 0;
    const current = (await kv.get(MEETINGS_KEY)) || [];
    if (!Array.isArray(current)) throw new MeetingStoreError('meetings_corrupt', 'The meetings store is not an array.');
    const outcome = await mutate(current);
    if (outcome === null || outcome === undefined) return null;
    if (!Array.isArray(outcome.meetings)) throw new MeetingStoreError('bad_mutation', 'updateMeetings: mutate must return { meetings }.');
    const json = JSON.stringify(outcome.meetings);
    if (Buffer.byteLength(json, 'utf8') > KV_MAX_REQUEST_BYTES - SAFETY_MARGIN_BYTES) {
      throw new MeetingStoreError('meetings_too_large', `Refusing to write 'meetings': ${Buffer.byteLength(json, 'utf8')} bytes is at the KV 10 MB request limit.`);
    }
    const stored = await kv.eval(MEETINGS_CAS_SCRIPT, [MEETINGS_KEY, MEETINGS_VERSION_KEY], [json, String(version)]);
    if (Number(stored) === 1) return outcome;
    await new Promise(resolve => setTimeout(resolve, 20 * (attempt + 1)));
  }
  throw new MeetingStoreError('meetings_write_conflict', 'The meetings store changed repeatedly while updating. Try again.');
}
...
export function captureIdentity(req, userId, transcript) {
  ...
  const source = captureSourceID(req.body?.source);
  return {
    id, userId, recordID, day, source,
    key: `capture-receipt:v1:${hash(userId)}:${id}`,
    fingerprint: hash(transcript.trim()),
    // One meeting per user per local day collects every capture from the watch/phone app; sources filed
    // apart (Glimpse) get their own day meeting via the suffix (lib/capture-sources.mjs).
    meetingID: `m_capture_day_${hash(userId).slice(0, 16)}_${day}${dailyMeetingSuffix(source)}`,
  };
}
...
/** "Sep 7, 2026" for a YYYY-MM-DD day. */
export function dayText(day) {
  const [year, month, date] = day.split('-').map(Number);
  return `${MONTHS[month - 1]} ${date}, ${year}`;
}

/** "Quick captures — Sep 7, 2026" (or "Glimpse — …"): the date tells one day's meeting from the next. */
export function dailyMeetingTitle(day, source = 'watch') {
  return `${captureSourceMeta(source).titlePrefix} — ${dayText(day)}`;
}
...
export function dailyMeeting(capture, { userId, transcript, captureSource, parsedBy }) {
  const meta = captureSourceMeta(captureSource ?? capture.source);
  return {
    id: capture.meetingID,
    userId,
    title: dailyMeetingTitle(capture.day, meta.id),
    sourceFileName: meta.file,
    transcript,
    date: capture.day,
    duration: null,
    participants: [],
    summary: meta.summary(dayText(capture.day)),
...
// KEYS[6] is the meetings version counter (`updateMeetings` above) for the same reason: a website
// meeting edit in flight retries instead of dropping the day's capture meeting.
// The day's transcript log (KEYS[4]) is appended to whenever it already exists, even when the
// meeting row is missing (a stale edit once removed it): the row is recreated, the log is kept.
...
-- Transcript log for the day: append this capture's entry to what earlier captures wrote. The
-- decision follows the log key itself, not the meeting row, so a row lost to a stale edit never
-- causes the day's earlier captures to be overwritten.
...
export async function commitCapture(kv, capture, { meeting, tasks, response, transcriptEntry }) {
  ...
  try {
    const result = await kv.eval(CAPTURE_COMMIT_SCRIPT,
      [capture.key, MEETINGS_KEY, 'tasks', `meeting:${meeting.id}:transcript`, TASKS_VERSION_KEY, MEETINGS_VERSION_KEY], args);
