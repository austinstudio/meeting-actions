 ...
 */
export async function extractWithGemini(text, { people = [] } = {}) {
  const model = genAI.getGenerativeModel({ model: GEMINI_MODEL });
  const result = await model.generateContent(getExtractionPrompt() + peopleDirectoryPrompt(people) + '\n' + text);
 ...
 */
export function buildTaskRecords(tasks, { userId, meetingId, sourceLabel, createdAt = new Date().toISOString(), defaultStatus = 'uncategorized' }) {
