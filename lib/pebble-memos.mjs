...
/** Hard cap on queue membership (records expire after 14 days anyway); trimmed atomically in `enqueue`. */
export const MAX_PENDING = 1000;
/** How many pending memos one GET /api/pebble/pending returns; bounds every loop below. */
...
/**
 * One atomic ZADD, then one atomic trim: ZREMRANGEBYRANK 0 -(MAX_PENDING+1) keeps exactly the newest MAX_PENDING
 * members however many writers race (a ZCARD-then-trim pair over-trimmed when two arrivals both saw the excess).
 */
async function enqueue(kv, userId, memo) {
  await kv.zadd(pendingSetKey(userId), { score: scoreFor(memo), member: memo.id });
  await kv.zremrangebyrank(pendingSetKey(userId), 0, -(MAX_PENDING + 1));
}
...
/**
 * Store a new memo: audio (if any), record, then queue membership. Returns
 *   'stored'    — new memo, queued;
 *   'repaired'  — the record already existed and is still pending, so it was re-queued (the state a crash between
 *                 the record write and the ZADD leaves behind; the Pebble app's retry repairs it). The phone still
 *                 needs a wake for it, so callers treat this like 'stored' for notifications;
 *   'duplicate' — the record exists and is already done; nothing to do.
 */
export async function storeMemo(kv, userId, memo, audioBody) {
  const existing = await kv.get(memoKey(userId, memo.id));
  if (existing) {
    if (existing.status !== 'pending') return 'duplicate';
    await enqueue(kv, userId, existing);
    return 'repaired';
  }
...
/** Pending memos, oldest first (one page of LIST_PAGE); ids whose record expired or is no longer pending are removed. */
export async function listPending(kv, userId) {
  await migrateLegacyPending(kv, userId);
  const ids = (await kv.zrange(pendingSetKey(userId), 0, LIST_PAGE - 1)) || [];
  const memos = [];
