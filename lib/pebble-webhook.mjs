...
/**
 * Minimal multipart/form-data parser (no dependency): every part with its headers and raw body Buffer.
 * Returns null when the content-type has no boundary.
 */
export function parseMultipart(buf, contentType) {
  ...
  while (pos !== -1) {
    ...
    parts.push({
      name: /name="([^"]*)"/i.exec(rawHeaders)?.[1] ?? null,
      filename: /filename="([^"]*)"/i.exec(rawHeaders)?.[1] ?? null,
      contentType: /content-type:\s*([^\r\n]+)/i.exec(rawHeaders)?.[1]?.trim() ?? null,
      body: buf.subarray(bodyStart, Math.max(bodyStart, bodyEnd)),
    });
    pos = next;
  }
  return parts;
...
/** Shape summary of a multipart body: part names, filenames, types, sizes, magic — not the bytes. */
export function summarizeMultipart(buf, contentType) {
  const parsed = parseMultipart(buf, contentType);
  if (!parsed) return { error: 'no boundary in content-type' };
  const parts = parsed.map(({ name, filename, contentType: type, body }) => {
