...
/** Headers worth recording. Authorization and cookies are never included. */
export function pickHeaders(headers = {}) {
  const out = {};
  for (const [k, v] of Object.entries(headers)) {
    if (!HEADER_ALLOW.test(k) || /authorization|cookie|signature|secret|token/i.test(k)) continue;
    out[k.toLowerCase()] = Array.isArray(v) ? v.join(', ') : String(v).slice(0, 300);
  }
  return out;
}
