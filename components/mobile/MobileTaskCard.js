  ...
  return (
    ...
    >
      ...
      <div className="flex items-start gap-2">
        ...
        <h3 className={`font-medium text-slate-900 dark:text-white ${large ? 'text-base leading-snug' : 'text-sm leading-tight line-clamp-2'}`}>
          {task.task}
        </h3>
      </div>
      ...
      {/* Meeting source */}
      {meeting && (
        <p className={`text-slate-500 dark:text-neutral-400 mt-1 ${large ? 'text-xs' : 'text-xs'}`}>
          From: {meeting.title}
        </p>
      )}
      ...
      <div className="flex items-center gap-3 mt-1.5 flex-wrap">
        ...
        {task.owner && (
          <span className="flex items-center gap-1 text-slate-500 dark:text-neutral-400 text-xs">
            <User size={12} />
            {task.owner}
          </span>
        )}
      ...
      {/* Context (large only — limited to 2 lines) */}
      {large && task.context && (
